...
def main(path, num_samples, alpha, beta, out_path):
    ...
    with pq.ParquetWriter(out_path, schema=schema) as writer:
        ...
        while i < num_samples:
            ...
            i+= batch_size
            print(f"Sampled {i} trees so far.")



    print(f"Sampled {i} trees: " + str(time.time() - start) + " seconds")
...
def draw_sample_bclt_cp(P, subtrees, dist, n_cells):
    ...
    alive_row = cp.concatenate([
        cp.ones(n_cells, dtype=bool),
        cp.zeros(n_cells - 1, dtype=bool)
    ])
    subtrees, prior_prob, norm_factor = sample_rec_cp(P, subtrees, dist, n_cells, 0, cp.tile(alive_row, (batch_size, 1)))
...
def sample_rec_cp(P, subtrees, dist, n_cells, iter, alive):
    """
    ...
    #base case
    if cp.sum(alive[0]) == 1:
        return subtrees, cp.full(batch_size, 1, dtype=np.float64), cp.full(batch_size, 1, dtype=np.float64)
    ...
    #apply choice batch wise

    cdf = cp.cumsum(prob_flat, axis=0)
    u = rng.random(batch_size) # REMEMBER TO REMOVE ASARRAY WHEN YOU SWITCH TO CUPY GENERATOR
    ind = cp.argmax(u[cp.newaxis, :] <= cdf, axis=0).astype(cp.int32)


    pair = cp.stack(cp.unravel_index(ind, (prob.shape[0], prob.shape[1])), axis=1)
    ...
    removed = (pair[:, 0], pair[:, 1])

    alive[batch_array,removed[0]] = 0
    alive[batch_array,removed[1]] = 0
    alive[:, n_cells + iter] = 1
    
    mask = ~alive
    batch, idx = cp.nonzero(mask) # returns a tuple of arrays of indices where mask is nonzero. 

    dist[n_cells + iter, idx, batch] = cp.nan
    dist[idx, n_cells + iter, batch] = cp.nan
    ...
    dist[removed[0], :, cp.arange(dist.shape[2])] = cp.nan
    dist[:, removed[0], cp.arange(dist.shape[2])] = cp.nan
    dist[removed[1], :, cp.arange(dist.shape[2])] = cp.nan
    dist[:, removed[1], cp.arange(dist.shape[2])] = cp.nan


    subtrees, prior_prob, norm_factor = sample_rec_cp(P, subtrees, dist, n_cells, iter + 1, alive)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-b", "--batch_size", type=int,
                        help="batch size", required=False, default=25)
    ...
    parser.add_argument("-di", "--divide", type=bool,
                        help="If True, use divide normalizaion with parameter divide_factor", required=False, default=True)
    parser.add_argument("-df", "--divide_factor", type=float,
                        help="divide_factor", required=False, default=10)

    args = parser.parse_args()

    #Global constants. TODO: These should probably be given as parameters to main instead because it could be confusing
    rng = cp.random.default_rng(seed=args.seed)
    eps = args.epsilon
    delta = args.delta
    coef = args.coef
    divide = args.divide
    ...
    batch_array = cp.arange(batch_size)
    ...
    if args.num_samples % batch_size != 0:
        raise Exception(
            f"num_samples ({args.num_samples}) must be divisible by batch_size ({batch_size}). batch_size can be set with -b or --batch_size (default 25)"
        )
    main(args.input_matrix, args.num_samples, args.alpha, args.beta, args.output)
